...
evt <- function(from, to, zone, area, camera){
  ...
  res_query <- dbGetQuery(data_handler, sprintf("select * from dc where (op16 = 20 AND cab32 = %d AND tick32 <= %d AND etick32 >= %d )", i_cab32, i_from, i_to))
  return_json_data = res_query
  ...
  repeat {
    calc_count = calc_count + 1
    tmp_dck_dcv = ''
    if (calc_count > lengths(return_json_data['cab64'])) {
      break
    } else if (calc_count == lengths(return_json_data['cab64'])) {
      tmp_dck_dcv = sprintf('{"dck": {"op16": %d, "cla": %d, "cab64s": "%s", "cab32s": "%s", "id64": %s, "id32": %d}, "dcv": %s}',
                            20,
                            as.numeric(list_cla16[[calc_count]]),
                            list_cab64s,
                            list_cab32s,
                            as.bigz(list_id64[[calc_count]]),
                            list_id32[[calc_count]],
                            list_dcv[[calc_count]]
      )
    } else {
      tmp_dck_dcv = sprintf('{"dck": {"op16": %d, "cla": %d, "cab64s": "%s", "cab32s": "%s", "id64": %s, "id32": %d}, "dcv": %s},',
    ...
    dck_dcv_items = paste(dck_dcv_items, tmp_dck_dcv)
  }
  
  result_json = sprintf('{"dcpref": {"cab64s": "%s", "op": %d, "ser": %d}, "items":[%s]}',
                        db_node_name,
                        as.numeric(op_val),
                        as.numeric(ser_val),
                        dck_dcv_items)
